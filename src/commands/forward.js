        ...
        try {
            ...
            if (duration && !isNaN(duration)) {
                if (music.player.state.position + duration * 1000 < music.current.info.length) {
                    ...
                    return msg.channel.send(util.embed().setDescription(`fastforwarded to ${util.millisToDuration(durationMs)}`));
                } else {
            ...
            if (!duration) {
                if (music.player.state.position + fdnumber * 1000 < music.current.info.length) {
                    music.player.seek(music.player.state.position + fdnumber * 1000);
                    msg.react("⏩");
                } else {
                    return msg.channel.send(util.embed().setDescription("❌ | The duration you provide exceeds the duration of the current track."));                
                }
