...
module.exports = {
    ...
    exec: async (ctx) => {
        ...
        if (from === null || to === null)
            return ctx.respond(util.embed().setDescription(`❌ | Missing args. Example usage e.g. \`${ctx.client.prefix}move 2 1\``));

        if (from === to || (isNaN(from) || from < 1 || from > music.queue.length) || (isNaN(to) || to < 1 || to > music.queue.length))
            return ctx.respond(util.embed().setDescription("❌ | Number is invalid or exceeds queue length."));

        const moved = music.queue[from - 1];

        util.moveArrayElement(music.queue, from - 1, to - 1);

        ctx.respond(util.embed().setDescription(`✅ | Moved **${moved.info.title}** to \`${to}\`.`));
    }
};
